...
class SmsUtility {
    ...
    public function send(
        array                             $pluginConfig,
        OrderStateMachineStateChangeEvent $event
    ): array {
        $response = null;
        $phone = null;
        $client = null;
        ...
        $billingAddress = $this->orderAddressRepository
        ...
        if (null !== $phone) {
            ...
            $params = (new SmsParams($text, $phone))
                ->setFrom($pluginConfig['from'] ?? null);
            $client = new Client($pluginConfig['apiKey'], 'shopware6');
            $response = (new SmsResource($client))->dispatch($params);
        }
    ...
    public static function getText(
        ...
        if (
            array_key_exists('signature', $pluginConfig)
            && mb_strlen($pluginConfig['signature'])
        ) {
